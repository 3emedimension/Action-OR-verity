    ...
    with get_db() as db:
        db.execute("""
            CREATE TABLE IF NOT EXISTS challenges (
                ...
                mode TEXT NOT NULL CHECK(mode IN ('soft','hot')),
                text TEXT NOT NULL,
...
def get_challenge():
    ...
    ctype = data.get("type")
    mode = data.get("mode")
    if ctype not in ("action", "verite") or mode not in ("soft", "hot"):
...
def list_challenges():
    ...
    with get_db() as db:
        rows = db.execute(
            "SELECT id, type, mode, text FROM challenges ORDER BY type, mode, id"
        ).fetchall()
...
def add_challenge():
    ...
    if ctype not in ("action", "verite") or mode not in ("soft", "hot") or not text:
